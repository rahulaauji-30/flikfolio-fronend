...
const Favourites = () => {
  ...
  const removeFromFavourite = async (id) => {
    try {
      const res = await axios.post(`${ENDPOINT}remove-from-favourite/1/${id}`);
      if (res.status === 200) {
        setError(false);
  ...
  const getMovieid = async () => {
    try {
      const res = await axios.get(`${ENDPOINT}get-favourites/1`);
      const movieIds = res.data.favourite_movies.map((movie) => movie.movie_id);
      getMovies(movieIds);
    } catch (error) {
